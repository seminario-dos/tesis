...
\begin{enumerate}
    ...
    \item La función \emph{Image-Handler} se ejercitó con 100 solicitudes de redimensionamiento para convertir la imagen del punto \#1 a una imagen de 50 pixeles de ancho y 60 de alto.
    \begin{enumerate}
    ...
    \item Se ejecutaron 100 solicitudes de redimensionamiento utilizando la versión original de la función \emph{Image-Handler}.
    \item Por último se compararon y analizaron los resultados del punto \#4 y \#5 para saber si, de acuerdo a lo obtenido, las simulaciones hechas sobre el modelo lograban caracterizar el rendimiento de la función Lambda.
\end{enumerate}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=17cm]{image-handler-small-img-extracted-repository-diagram.png}
  \caption{\emph{Image-Handler} publicando eventos de rendimiento al servicio AWS X-Ray}
...
Los SEFFs generados por PMX no eran lo suficientemente detallados y cuando se realizaban simulaciones, los resultados no lograban acercarse a la observado en la función Lambda. Por esta razón fue se recurrió al trabajo del punto \#3, para obtener datos ``crudos'' del rendimiento de la función Lambda e incluir esto en los SEFFs del modelo como distribuciones de frecuencia.
